...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const pdfBuffer = await renderToBuffer(ReportCardPDF({ student, scores: scoresForExam, exam, ranking: position, total, average, grade, remark }));
  return new NextResponse(pdfBuffer, { headers: { 'Content-Type': 'application/pdf', 'Content-Disposition': `inline; filename=report_${student.admissionNumber}.pdf` } });
}